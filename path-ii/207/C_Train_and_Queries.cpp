...
void f() {
    ...
    vi a(n); cin >> a;
    /* 
    find the first and the last position for each id
     */
    int pos = 0;
    hashmap<int,int> first_pos;
    hashmap<int,int> last_pos;
    for(auto&x:a) {
        if (!first_pos.count(x)) first_pos[x]=pos;
        last_pos[x]=pos;
        pos++;
    }
    ...
    while (q--) {
        cin >> l >> r;
        if (!first_pos.count(l) || !last_pos.count(r))
            cout << "NO\n";
        else {
            int l_pos = first_pos[l];
            int r_pos = last_pos[r];
            if (l_pos < r_pos) {
                cout << "YES\n";
            } else {
                cout << "NO\n";
