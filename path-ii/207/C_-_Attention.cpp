...
void f() {
    ...
    int facing_east_partial = 0;
    int max_to_center = 1e12;
    for (int i = 0; i < n; i++) {
        char x = s[i];
        int prev_facing_west = i-facing_east_partial;
        if (x=='E') facing_east_partial++;
        int not_facing_center = (facing_east-facing_east_partial)+prev_facing_west;
        chmin(max_to_center, not_facing_center);
    }
    cout << max_to_center << endl;
}