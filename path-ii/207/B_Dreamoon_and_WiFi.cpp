...
void f() {
    ...
    int fi_pos = 0;
    for(auto&x:s) fi_pos+=(x=='+'?1:-1);
    
    int my_pos = 0, moves = 0;
    for(auto&x:t)
        if (x=='?') moves++;
        else my_pos+=(x=='+'?1:-1);
    
    int dist = fi_pos-my_pos;
    double ans;

    // situations those cannot reach
    if ((dist+moves)%2 || moves<abs(dist)) ans = 0;
    else {
        //moves needed toward the distance m is abs(distance)+(moves-abs(distance))/2
        int m = (moves+abs(dist))/2;
        //answer is C(moves,m)/(1<<moves) #combinatorics
        int c = 1;
        rep(i,m) c*=moves-i;
        FOR(i,2,m+1) c/=i;
