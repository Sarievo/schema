...
void f() {
    ...
    // set<string> occured;
    // rep(i, n) {
    //     occured.insert(a[i]+a[i]);
    //     rep(j, n) {
    //         if (i != j) occured.insert(a[i]+a[j]);
    //     }
    // }
    // string ans = "";
    // for (auto&x:a)
    //     if (occured.count(x)) ans.push_back('1');
    //     else ans.push_back('0');
    // cout << ans << endl;
    unordered_set<string> m;
    vc<bool> valid(n,false);
    ...
    for(int i = 0; i < n; ++i) {
        ...
        if (x.size()==1) continue;
            ...
            for (int pos = 1; pos < x.size(); pos++) {
                string substr1 = x.substr(0, pos);
                string substr2 = x.substr(pos);
                if (m.count(substr1) && m.count(substr2)) {
                    valid[i] = true;
                    break;
                }
            }
