...
void f() {
    ...
    int n = s.size();
    /* 
    if the s only consists of 0, it's 1
    if the s only consists of 1, it's 0
    otherwise, how many non 1 segments does it forms? 
     */
    int ones = 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (non_1_segs >= 2) break;
    }
    cout << min(non_1_segs, 2ll) << endl;
}