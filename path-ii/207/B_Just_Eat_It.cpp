...
int maximum_subarray_sum(int start, int end, vi&a) {
    // kadane algorithm
    int cur_sum = 0, max_sum = 0;
    for(int i = start; i < end; ++i) {
        cur_sum = max(cur_sum + a[i], 0ll);
...
void f() {
    int n; cin >> n;
    vi a(n); cin >> a;
    /* find out if the sum of any segment is greater than the sum of the whole */
    int sum1 = maximum_subarray_sum(0,n-1,a);
    int sum2 = maximum_subarray_sum(1,n,a);
    int sum = acc(a);
    cout << (max(sum1,sum2) < sum ? "YES":"NO") << endl;
}