...
void f() {
    int W, H, N; cin >> W >> H >> N;
    /* we have 4 coordinates of the white space.., is 2 coordinates enough for describing it? */
    int x1 = 0, y1 = 0, x2 = W, y2 = H;

    FOR(i, 0, N) {
        int x, y, a; cin >> x >> y >> a;
        if (a == 1) {
            // all x lesser than xi is painted. this means to limit x1 to xi.
            x1 = max(x1, x);
        } else if (a == 2) {
            // all x bigger than xi is painted, this means to limit x2 to xi.
            x2 = min(x2, x);
        } else if (a == 3) {
            // etc.
            y1 = max(y1, y);
    ...
    if (x2 <= x1 || y2 <= y1) cout << 0 << endl; // all is painted
