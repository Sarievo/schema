...
void pre_calc() {
    int ord = 2;
    while (1) {
        int next = pre.back() + (ord *2 + (ord++ - 1));
        if (next > int(1e9+7)) return;
        pre.push_back(next);
    }
}
...
void f() {
    int n; cin >> n;
    int id = upper_bound(all(pre), n) - pre.begin();
    int cost = id ? pre[id-1] : 0;
    int times = 0;
    while (cost) {
        ++times;
        // cout << cost << " ";
        n -= cost;
        id = upper_bound(all(pre), n) - pre.begin(); // binary search
        cost = id ? pre[id-1] : 0;
    }
    // cout << endl;
    cout << times << endl;
