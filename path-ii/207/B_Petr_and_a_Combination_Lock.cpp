...
bool turn(vi&a, int step, int cur_deg) {
    if (step == sz(a)) return (cur_deg % 360) == 0;
    else return turn(a, step+1, cur_deg+a[step]) || turn(a, step+1, cur_deg-a[step]);
}

void f() {
    int n; cin >> n;
    vi a(n); cin >> a;
    cout << (turn(a, 0, 0) ? "YES" : "NO") << endl;
}