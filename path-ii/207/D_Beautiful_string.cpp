/** 
 * Sarievo.
 * URL: https://codeforces.com/group/a8Whn6p0N1/contest/387841/problem/D
 */
...
void f() {
    ...
    vc<pi> freq;
    for(auto&p:fmap) freq.push_back(mp(p.b,p.a));
    std::sort(all(freq),[](pi&i, pi&j){return i.first > j.first;});
    
    vc<char> t;
    while(freq[0].a--)
        t.push_back(freq[0].b);

    vc<char> next;
    int i = 1;
    while (i < freq.size()) {
        while (next.size() < t.size() && freq[i].a > 0) {
            next.push_back(freq[i].b);
            freq[i].a--;
        }
    ...
    // cout << next << endl;
    for(auto&ch:t) cout << ch; cout << endl;
}