...
void f() {
    ...
    int sum_a = acc(a);
    int sum_b = acc(b);
    int row_1_sum = 0, row_2_sum = 0, min_sum = 1e12;
    
    rep(i,m) {
        row_1_sum += a[i];
        row_2_sum += (i ? b[i-1] : 0);
        int row_1_rem = sum_a-row_1_sum;
        int row_2_rem = sum_b-row_2_sum;
        int bob_sum = max(row_1_rem, row_2_sum);
        chmin(min_sum, bob_sum);
    }
    cout << min_sum << endl;
}