...
void f() {
    int n; cin >> n;
    vi a(n); cin >> a;
    hashmap<int,int> inexp;
    for(auto&x:a) inexp[x]++;
    int groups = 0;
    vc<pi> inexp_a;
    for (auto&p:inexp) {inexp_a.push_back(p);}
    sort(all(inexp_a),[](pi&a, pi&b){return a.a < b.a;});
    int prev_remain = 0;
    for (auto&p:inexp_a) {
        int people = p.b + prev_remain;
        groups += people / p.a;
        prev_remain = people % p.a;
    }
    cout << groups << endl;
}