...
void solve() {
    ...
    afor(i, m) {
        cin >> a2[i];
        if (a2[i] > maxB) maxB = a2[i];
    }

    pair<int, int> winner = {0, 1};
    if (maxA >= maxB) winner.x = 1;
    if (maxB >= maxA) winner.y = 0;

    cout << (winner.x ? "Alice" : "Bob") << endl;
    cout << (winner.y ? "Alice" : "Bob") << endl;
}
