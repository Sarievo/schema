...
void f() {
    ...
    if the strings are of equal length, they must be equal, 
    otherwise, the later one must contain the first one
     */
    sort(all(a), [](string&s, string&t){return s.length() < t.length();});
    bool matched = true;
    for (int i = 0; i < n-1; i++) {
        if (!contains(a[i], a[i+1])) {
            matched = false;
            break;
        }
    }
