...
void f() {
    ...
    int min_non_10_quiz = INT_MAX;
    int sum_score = 0;
    for (auto &x : a) {
        sum_score += x;
        if (x % 10 != 0) chmin(min_non_10_quiz,x);
    }
    if (min_non_10_quiz==INT_MAX) {
        cout << 0 << endl; // every num is a multiple of 10!
    } else {
        if (sum_score % 10 != 0) {
            cout << sum_score << endl; // already optimal
        } else {
            cout << sum_score-min_non_10_quiz << endl;
        }
    }
}