...
int ceil_divistion(int a, int b) {
    return (a+b-1) / b;
}

void f() {
    int n,a,b; cin >> n >> a >> b;
    int quality_days = (n+1) / 2;
    int total_days = (quality_days/a) * (b+a);
    total_days += (quality_days%a) ? quality_days%a : -b;
    cout << max(n, total_days) << endl;
}