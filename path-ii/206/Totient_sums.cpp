...
//!<---------- start of define !<--------------------------
using ll=long long;
#define int ll // <- prevent overflow
...
#define fill(a) iota(all(a),int(0))
//!<---------- end of define !<----------------------------
...
signed main() {
    IOS;
    iota(all(phi),0);
    totient(); // precalculate Euler's totient
    prefix_sum(); // calc prefix sum for O(1) retrieval
