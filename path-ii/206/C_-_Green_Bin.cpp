...
void f() {
    int n; cin >> n;
    vc<string> words(n); cin >> words;
    /* print number of pairs of anagrams */
    /* 
        find freq map of every word, 
        encode them into string, 
        push the string into a freq map,
        if the entry has +1 value, the answer += v*(v-1)
     */
