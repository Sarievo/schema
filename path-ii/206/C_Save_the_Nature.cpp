/** 
 * Sarievo.
 * URL: https://codeforces.com/contest/1223/problem/C
 */
...
int calc(vi& p, int len, int x, int a, int y, int b) {
    int ans = 0;
    int cX = 0, cY = 0, cXY = 0;
    for (int i = 1; i <= len; i++) {
        if (!(i%a) && !(i%b)) cXY++;
    ...
    rep(i, cXY) ans+=p[i]*(x+y);
    rep(i, cX) ans+=p[cXY+i]*x;
    rep(i, cY) ans+=p[cXY+cX+i]*y;
    return ans;
}

void f() {
    int n; cin >> n;
    vi p(n); cin >> p; for(auto&x:p) x/=100;
