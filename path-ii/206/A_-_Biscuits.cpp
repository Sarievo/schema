...
void f() {
    ...
    /* 
    since it's congruent to either 1 or 0, the numbers do not matter except their parity
     */
    int odds = 0;
    for (auto&x:a) if (x%2) odds++;
    int evens = n-odds;
    /* 
    two odds can be added to make a even, and odd + even is odd
     */
    int total = 2;
    if (!odds) {
        if (!k) total = pow(total,n);
        else total = 0;
    } else {
        total = pow(total,n-1);
    }
