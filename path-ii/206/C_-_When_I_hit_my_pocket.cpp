...
void f() {
    int k, a, b; cin >> k >> a >> b;
    /* 
    now, if the b is lesser equal then a + 2, sunuke may never found exchange to jpy profitable
    otherwise, sunuke will always exchange b biscuit with c biscuit, ans so on
     */
    if (b <= a+2) {
        cout << k+1 << endl;
    } else {
